...
Hedera is a layer-1 [blockchain implemented in Java programming language](https://github.com/hashgraph/hedera-services/tree/develop/hedera-node). It uses a custom consensus mechanism called [Hashgraph](https://hedera.com/learning/hedera-hashgraph/what-is-hashgraph-consensus) which is a subset of a better known PoS. 

The Hedera Token Service (HTS) is a platform released by Hedera Hashgraph on February 9th,2021. It is a public distributed ledger technology (DLT) network. HTS enables the creation, issuance, and management of digital tokens, including non-fungible tokens (NFTs).
...
In traditional EVM compatible NFTs the collection address distiguishes a group of NFTs while each token has its unique `tokenId` differentiating it from the other tokens of the same colleciton.
...
touch XPNFTHTS.sol
```
Place the following code to the newly created file.
...
Contract's global variables and structures are usually placed before the contructor or the initialization function. They represent the long-term memory of the contract. The values reside on every node of the blockchain and therefore writing to the storage is the most expencive operation.
...
Minting is another word for coining or creating the tokens. During minting new records are made in the contrct storage. Here we add the `onlyOwner` modifier, because only the XP.NETWORK bridge must mint wrapped NFTs on Hedera. If you're using this code to deploy your own collection, replace the modifier with something relevant to your usecase.
...
If an NFT arrived to Hedera from a foreign chain, it must be burnt, once it is sent to the chain of origin or to another foreign chain. Therefore, the wrapped contract must implement the `burnFor` funciton.
...
For your convenience we've created a repo for Custom HTS NFT contract deployment compatible with the XP.NETWORK bridge.
